...
for m = ProteinConc
    ...
    while Equilibrium == 0
        ...
        a = [a_If(Events+1),a_SCf(Events+1),a_DCf(Events+1),a_r(Events+1)]; %vectorized propensity functions
        R = [rand,rand,rand,rand];
        tau = (1./a).*log(1./R); %times to each reaction
        dt(Events+1) = min(tau);
        j(Events+1) = find(tau == min(tau));  %first reaction to occur
